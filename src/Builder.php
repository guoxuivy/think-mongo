...
class Builder
{
    ...
    protected function parseSet($data, $options)
    {
        ...
        foreach ($data as $key => $val) {
            $item = $this->parseKey($key);
            if (is_array($val) && isset($val[0]) && in_array(strtolower($val[0]), ['$inc', '$set', '$unset', '$push', '$pushall', '$addtoset', '$pop', '$pull', '$pullall'])) {
                ...
                if ($value instanceof \Closure) {
                    ...
                    if (strpos($field, '|')) {
                        ...
                        foreach ($array as $i=>$k) {
                            // where('business_a|business_b',[1,3]) 模式 类似mysql or 查询
                            if(is_array($value) && count($value)==1){
                                // 扩展使用不同查询条件
                                $val = $value[0];
                                $filter['$or'][] = $this->parseWhereItem($k, $val[$i]);
                            }else{
                                //使用相同查询条件
     ...
     * 聚合查询命令 
     * db.store.aggregate( [
     *     { $match:{ surface_rent_currency : { $gt : 0} } },
     *     { $group:{ _id: "$park_id", count: { $sum: 1 },max:{$max:"$surface_rent"} } },
     *     { $sort:{"modify_date":-1}}
     * ]);
     * 将group与order参数完全开放给上层
     * @access public
     * @param array $options 参数
     * @param array $extra   分组字段+指令和字段
     * @return Command
     */
    public function group($options, $extra)
    {
        list($group, $sort) = $extra;
        $pipeline          = [
            ['$match'   => (object) $this->parseWhere($options['where'])],
        ];
     ...
     */
    public function groupCount($options, $extra)
    {
        list($group) = $extra;
        $pipeline          = [
            ['$match'   => (object) $this->parseWhere($options['where'])],
     ...
     * @return [type]          [description]
     */
    public function lookup($options, $extra)
    {
        list($from, $localField, $foreignField, $as, $group) = $extra;
        // $lookup          = ["from" => "park","localField" => "park_id","foreignField" => "park_id","as" => "park_docs"];
     ...
     * @return [type] [description]
     */
    public function pipeline($options,$pipeline=[]){
        foreach ($pipeline as &$v) {
            foreach ($v as $key => &$value) {
                if($key == '$match' && $value=="tp_where"){
                    $value = (object) $this->parseWhere($options['where']);
                }
            }
        }
        $cmd = [
