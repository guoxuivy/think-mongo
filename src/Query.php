...
class Query
{
    ...
    /**
     * 简单连表查询 需要索引支持
     * (new Store())->where("park_docs.code","EYQ171023041")->field('code,contact,phone,park_docs')->order("phone",'desc')->limit(3)->lookup('park','park_id','park_id','park_docs');
     * @access public
     * @param string $from 需要连接的表
     * @param string $localField  关联id
     * @param string $foreignField 关联id
     * @param string $as  给被关联表起别名
     * @param string $group  是否再次分组 $group   => ["_id"=>1,"count"=>['$sum'=>1]]
     * @return mixed
     */
    public function lookup($from, $localField, $foreignField, $as,$group=false)
    {
        $options = $this->parseExpress(); //清空option
        $command = $this->builder->lookup($options, [$from, $localField, $foreignField, $as, $group]);
    ...
    {
        $options = $this->parseExpress(); //清空option
        $command = $this->builder->pipeline($options, $pipeline);
        $result =  $this->command($command);
        return $result;
    }
     ...
     * 聚合查询
     * @access public
     * @param array $group 聚合数组
     * @param array $sort  排序规则数组
     * @return mixed
     */
    public function group($group,$sort=null)
    {
        $options = $this->parseExpress(); //清空option
        $command = $this->builder->group($options, [$group,$sort]);
        $result =  $this->command($command);
        return $result;
        // return isset($result[0]['result']) ? $result[0]['result'] : false;
    }
    public function groupCount($group,$clean_options = true)
    {
        if(!$clean_options){
        ...
        $command = $this->builder->groupCount($options, [$group]);
        $result =  $this->command($command);
     ...
     * @return $this
     * where('business_a|business_b',[1,3]) 类似mysql or 查询实现
     */
    ...
    {
        $pk = !empty($this->pk) ? $this->pk : $this->getConfig('pk');
